...
root_trace='/srv/research/benchmarks/champsim/dp3_traces/'
...
#ベンチマークリスト取得
trace_list=(`cat dpc3_traces_list.txt`) 
i=0
...
do 
    if [ $i -gt 8 ]; then
        ...
        bin/champsim --warmup_instructions $warmup_instruction --simulation_instructions $simulation_instruction $root_trace$trace > $path_to_results/$sim_result.txt
